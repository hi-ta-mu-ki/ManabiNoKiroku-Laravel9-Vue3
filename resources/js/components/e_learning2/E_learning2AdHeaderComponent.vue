<template>
  <div class="container-fluid bg-dark mb-3">
    <nav class="navbar navbar-dark">
      <span class="navbar-brand mb-0 ms-5 h1 text-white">まなびのきろく３　ユーザ管理</span>
      <div>
        <router-link v-bind:to="{name: 'ad.userlist'}">
          <button class="btn btn-success me-1 text-white">ユーザ一覧</button>
        </router-link>
        <router-link v-bind:to="{name: 'ad.usercreate'}">
          <button class="btn btn-success me-1 text-white">ユーザ追加</button>
        </router-link>
        <router-link v-bind:to="{name: 'ad.userimport'}">
          <button class="btn btn-success me-1 text-white">CSV インポート</button>
        </router-link>
