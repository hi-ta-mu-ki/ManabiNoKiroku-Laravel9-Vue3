  ...
  <div class="container-fluid bg-primary mb-3 text-white">
    <nav class="navbar navbar-primary">
      ...
      <div>
        ...
        <button class="btn btn-success text-white">グループ設定</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.member'}">
        <button class="btn btn-success text-white">クラス生徒設定</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.record'}">
          <button class="btn btn-success text-white">生徒個人記録</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.setting'}">
          <button class="btn btn-success text-white">問題実施設定</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.questionlist'}">
          <button class="btn btn-success text-white">問題一覧</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.questioncreate'}">
          <button class="btn btn-success text-white">問題作成</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.questionimport'}">
          <button class="btn btn-success text-white">問題 CSV インポート</button>
        </router-link>
        <router-link v-bind:to="{name: 'tc.join'}">
          <button class="btn btn-success text-white">クラスに参加</button>
        </router-link>
